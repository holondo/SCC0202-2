...
int atualiza_Altura(No *n)
{
    ...
    if(n->dir == NULL && n->esq == NULL) n->altura = 0;

    else n->altura = atualiza_Altura(n->dir) >= atualiza_Altura(n->esq) ? altura_Sub(n->dir)  : altura_Sub(n->esq) ;
    return n->altura;
}
...
int fator_Sub(No *n)
{
    ...
    return (altura_Sub(n->dir) - altura_Sub(n->esq));
}
No* esq(No *n)
{
...
No* balancea_AVL(Colecao *c, No *n)
{
    ...
    if( fator_Sub(n) == 2 && fator_Sub(n->dir) >= 0 && fator_Sub(n->dir) < 2)return esq(n);//Rotação p/ esquerda
